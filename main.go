...
func checkMonth(botToken, chatID string) {
	resp, err := insecureClient.Get("https://www.atsenergo.ru/results/market/calcfacthour")
	if err != nil {
		if !siteError && chatID != "" {
			siteError = true
			sendTelegramMessage(botToken, chatID, fmt.Sprintf("Ошибка при запросе сайта: %v", err))
		}
		return
	}
	...
	if err != nil {
		if !siteError && chatID != "" {
			siteError = true
			sendTelegramMessage(botToken, chatID, fmt.Sprintf("Ошибка при парсинге HTML: %v", err))
		}
		return
	}
